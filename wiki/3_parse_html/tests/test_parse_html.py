"""Tests for the demo_parse_html pipe."""
...
def load_pipe():
    ...
    from wiki._module_loader import load_pipe_package

    return load_pipe_package(
        PIPE_DIR,
        alias="test_3_parse_html_pkg",
    )
...
class TestTrailingEmptyCells:
    ...
    def test_wide_table_no_hang(self) -> None:
        from demo_parse_html.html_tool.converter.md import (
...
class TestRestoreLocalPaths:
    def test_strips_resolved_image_url(self) -> None:
        from pipes.demo_parse_html import restore_local_paths
...
SNAPSHOT_CASES = [
    (
        "source-html/articles/cooking.html",
        "markdown/articles/cooking.md",
        {},
    ),
    (
        "source-html/news/reuters-jesse-jackson.html",
        "markdown/news/reuters-jesse-jackson.md",
        {},
    ),
    (
        "source-html/wiki/xiaomi-wiki.html",
        "markdown/wiki/xiaomi-wiki-noref.md",
        {"remove_ref": True},
    ),
]
...
class TestSnapshotCases:
    @pytest.mark.parametrize(
        "html_file,expected_file,config",
        SNAPSHOT_CASES,
        ids=[c[0] for c in SNAPSHOT_CASES],
    )
    def test_pipe_matches_snapshot(
        self,
        html_file: str,
        expected_file: str,
        ...
        expected_path = EXAMPLES_DIR / expected_file

        assert html_path.exists(), f"Missing fixture: {html_path}"
        assert expected_path.exists(), (
            f"Missing snapshot: {expected_path}"
        )

        pipe = load_pipe()
        ctx = make_context(
            "parse_html", config=config or None,
        )
        batch = {
            "id": [html_file],
            "info": ["{}"],
        ...
        actual = VERSION_LINE_RE.sub("", result["data"][0])
        expected = VERSION_LINE_RE.sub(
