#!/usr/bin/env python3
"""Run 1_html_collect_image_urls_lance locally on a Lance dataset."""
...
PIPE_NAME = "stage1_html_collect_image_urls_lance"
...
def _worker_init(root_str: str, dataset: str, dataset_dir_str: str) -> None:
    ...
    _WORKER_CTX = _PipeContext(
        ...
        config={"dataset_dir": dataset_dir_str},
    )
...
def run(
    ...
    *,
    ...
    ctx = PipeContext(
        ...
        config={"dataset_dir": str(src_dataset_dir)},
    )

    print(f"source: {src_dataset_dir}")
    print(f"target: {dst_dataset_dir}")
    print(f"rows: {total}, batch_size: {batch_size}, workers: {workers}")
    ...
    if workers <= 1:
        ...
        with cf.ProcessPoolExecutor(
            ...
            initargs=(str(ROOT), src_dataset_dir.name, str(src_dataset_dir)),
        ) as pool:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=ROOT / "workspace" / "html_lance" / "wiki_0320_en_has_pic_url_indexed",
    )
