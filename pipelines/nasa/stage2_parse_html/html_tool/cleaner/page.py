...
AD_KEYWORDS = re.compile(
    r"(?<![a-z0-9\-])"
    r"(ad|ads|advert|banner|cookie|consent|copyright|disclaimer|feedback|"
    r"gform|gravity|marketing|modal|more-stories|newsletter|popup|promo|"
    r"related-articles|related-posts|related-stories|related|share-tools|share|sidebar|"
    r"social|subscribe|sponsor|breadcrumb|sponsored)"
    r"(?![a-z0-9\-])",
    re.IGNORECASE,
)
...
FORUM_UI_KEYWORDS = re.compile(
    ...
    r"external-links|catlinks|trust-badge|global-nav|hds-single-form-embed|"
    r"printfooter)"
    r"(?![a-z0-9\-])",
    re.IGNORECASE,
)
...
CSS_MODULE_NOISE_RE = re.compile(
    r"^(toolbar|sidebar|banner|cookie|consent|subscribe|newsletter|feedback|"
    r"ad|ads|advert|navigation|navbar|comment|form|popup|modal|sign-off)-module(?![a-z0-9\-])",
    re.IGNORECASE,
)
...
TESTID_NOISE_WORDS = frozenset({
    ...
    "pagination", "licence", "license", "feedback", "form", "forms",
    "share", "social", "related", "popup", "modal", "global", "footer",
})
...
CONTENT_XPATH: list[list[str]] = [
    ...
    [
        ...
        f".//*[{cls_xpath('usa-article-content')}]//*[{cls_xpath('entry-content')}]",
        f".//*[{cls_xpath('wp-block-post-content')}]",
    ...
    [
        ...
        f".//*[{cls_xpath('usa-article-content')}]",  # NASA WordPress article wrapper
