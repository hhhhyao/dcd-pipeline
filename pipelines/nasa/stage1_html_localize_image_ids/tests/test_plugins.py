...
from plugins import nasa_production
...
def test_nasa_plugin_normalizes_wordpress_and_science_assets_and_rewrites_img() -> None:
    html = (
        '<picture><source srcset="/wp-content/uploads/foo/image-750.webp?w=750 750w, '
        '/wp-content/uploads/foo/image-1500.webp?w=1500 1500w">'
        '<img src="/wp-content/uploads/foo/image-750.jpg?w=750" '
        'data-srcset="/wp-content/uploads/foo/image-750.webp?w=750 750w">'
        "</picture>"
    )
    assert nasa_production.extract_img_urls_from_html(html) == [
        "/wp-content/uploads/foo/image-750.jpg?w=750",
    ]
    assert (
        nasa_production.normalize_image_url("/wp-content/uploads/foo/image-750.jpg?w=750")
        == "www.nasa.gov/wp-content/uploads/foo/image-750.jpg?w=750"
    )
    assert (
        nasa_production.normalize_image_url(
            "https://assets.science.nasa.gov/dynamicimage/assets/science/foo/image-750.jpg?w=750",
        )
        == "assets.science.nasa.gov/dynamicimage/assets/science/foo/image-750.jpg?w=750"
    )
    assert (
        nasa_production.normalize_image_url("./assets/foo/image-750.jpg")
        == "nasa-asset/foo/image-750.jpg"
    )

    rewritten = nasa_production.rewrite_html(
        html,
        {
            "/wp-content/uploads/foo/image-750.jpg?w=750": [
