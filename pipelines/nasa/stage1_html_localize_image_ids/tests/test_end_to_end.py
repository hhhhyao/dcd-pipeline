...
def test_map_rewrites_nested_dcd_batch_and_preserves_images() -> None:
    image_ref_id = "img-1_refhash"
    image_url = "https://www.nasa.gov/wp-content/uploads/example.jpg?w=100"
    batch = {
        "text": {
            "id": ["text-1"],
            "data": [f'<html><body><img src="{image_url}"></body></html>'],
            "info": [
                json.dumps({
                    "url": "https://www.nasa.gov/news-release/example/",
                    "image_refs": {
                        image_ref_id: {
                            "image_url_ori": image_url,
                        },
                    },
                }),
            ],
        ...
        "image": {
            ...
            "info": [[json.dumps({"width": 100, "height": 80})]],
            "data": [["{}"]],
            "tags": [[[]]],
        },
    }
    ...
    out = pipe_module.map(batch, ctx)

    assert out["text"]["id"] == ["text-1"]
    assert 'src="images/img-1"' in out["text"]["data"][0]
    assert f'_image_ref_id="{image_ref_id}"' in out["text"]["data"][0]
    text_info = json.loads(out["text"]["info"][0])
    assert text_info["__defined__"]["links"]["images"] == [{"id": "img-1"}]
    assert out["image"]["id"] == ["img-1"]
    assert out["image"]["image_bytes"] == [b"image-bytes"]
