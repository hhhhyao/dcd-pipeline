...
_LABEL_SIZES_CACHE: dict[str, dict[str, tuple[int, int]]] = {}
...
def _resolve_labels_path(ctx: PipeContext) -> tuple[str, Path] | None:
    ...
    if ctx.volumes and ctx.volumes.get("dataset") is not None:
        dataset_dir = Path(ctx.volumes["dataset"])
...
def _load_label_sizes(labels_path: Path) -> dict[str, tuple[int, int]]:
...
def _get_label_sizes(ctx: PipeContext) -> dict[str, tuple[int, int]]:
    resolved = _resolve_labels_path(ctx)
    if resolved is None:
        return {}
    cache_key, labels_path = resolved
    cached = _LABEL_SIZES_CACHE.get(cache_key)
    if cached is not None:
        return cached
    cached = _load_label_sizes(labels_path)
    _LABEL_SIZES_CACHE[cache_key] = cached
    return cached
...
def map(batch: dict[str, list[Any]], ctx: PipeContext) -> dict[str, list[Any]]:
    ...
    label_sizes = _get_label_sizes(ctx)
