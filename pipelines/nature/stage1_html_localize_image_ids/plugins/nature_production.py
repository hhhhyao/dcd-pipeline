"""Nature-specific HTML image extraction, normalization, and rewriting."""
...
def normalize_image_url(url: str) -> str:
    """Normalize Nature image URLs for matching HTML attrs to text image_refs."""
    ...
    if value.startswith("assets/"):
        return f"nature-asset/{unquote(value.removeprefix('assets/'))}"

    parsed = urlparse(value if not value.startswith("//") else f"https:{value}")
    path = parsed.path or ""
    if "/assets/" in path:
        return f"nature-asset/{unquote(path.split('/assets/', 1)[1])}"
