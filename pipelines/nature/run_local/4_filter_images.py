...
import pipelines.nature.stage4_filter_images as PIPE_MODULE  # noqa: E402
try:
...
PIPE_NAME = "nature_stage4_filter_images"
...
def _call_pipe_map(
    mm_batch: dict[str, dict[str, list[Any]]],
    ctx: PipeContext,
) -> dict[str, list[Any]]:
    in_batch = multimodal_from_dict(mm_batch)
    out_batch = call_fn(pipe_map, in_batch, ctx, "dict")
    return {
        name: out_batch.column(name).to_pylist()
        for name in out_batch.schema.names
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("/root/zhouyiren/data/interleaved/nature/openai_lance/nature_test_v1_openai"),
    )
    parser.add_argument(
        "dst_dataset_dir",
        nargs="?",
        type=Path,
        default=Path("/root/zhouyiren/data/interleaved/nature/openai_lance/nature_test_v1_openai_filtered"),
    )
