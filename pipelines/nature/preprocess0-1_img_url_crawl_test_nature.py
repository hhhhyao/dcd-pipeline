#!/usr/bin/env python3
"""Collect image URLs from Nature article pages listed by preprocess0-0."""
...
DEFAULT_INPUT_URLS = Path("/root/zhouyiren/data/interleaved/nature/url_raw/urls.txt")
DEFAULT_OUTPUT_DIR = Path("/root/zhouyiren/data/interleaved/nature/img_url_raw")
USER_AGENT = (
    "dcd-pipeline-nature-img-url-crawler/0.1 "
    "(article image URL collection; +https://www.nature.com/)"
)
...
IMAGE_CONTEXT_TAGS = {"amp-img", "img", "source"}
IMAGE_VALUE_ATTRS = {
    "content",
    "data-hires",
    "data-lazy-src",
...
def normalize_image_url(raw_url: str, *, page_url: str) -> str | None:
    ...
    path = parsed.path or "/"
    image_like = bool(IMAGE_EXT_RE.search(path)) or host in TRUSTED_IMAGE_HOSTS
    if not image_like:
        return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect image URLs from Nature article URLs generated by preprocess0-0.",
    )
