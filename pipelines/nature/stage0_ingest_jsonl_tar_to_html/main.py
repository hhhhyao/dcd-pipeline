#!/usr/bin/env python3
"""Ingest Nature part*.jsonl + part*.tar source into Lance datasets.
...
def extract_html_meta(html: str) -> dict[str, object]:
    """Extract canonical URL and title from Nature HTML."""
    ...
    if m:
        ...
        result["title"] = re.sub(r"\s*[-|]\s*Nature\s*$", "", raw_title)
    result["tags"] = []
    return result
...
def _iter_remote_batches(
    ...
    *,
    ...
    """Yield DCD multimodal batches from raw Nature jsonl+tar source files."""
    pairs = _find_pairs(src_dir)
    line_counts = [_count_non_empty_lines(jf) for jf, _ in pairs]
    total_articles = sum(line_counts)
    text_written = 0
...
def _source_dir_for_ctx(ctx: Any, config: dict[str, Any]) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    ...
    tmp = tempfile.TemporaryDirectory(prefix="nature_stage0_raw_")
    try:
...
def ingest(ctx: PipeContext) -> Path | Iterator[dict[str, dict[str, list[Any]]]] | None:
    """Ingest raw Nature jsonl+tar source into a Lance dataset directory."""
    logging.basicConfig(
        level=logging.INFO,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Convert Nature part*.jsonl + part*.tar to Lance datasets",
    )
