...
def _write_one_article_fixture(src_dir: Path) -> tuple[bytes, dict[str, object]]:
    ...
    image_url_ori = "https://media.nature.com/example.jpg"
    entry: dict[str, object] = {
        "html": "<html><head><title>Example | Nature</title></head><body>hello</body></html>",
        "url": "https://www.nature.com/articles/raw",
...
def test_run_streaming_preserves_svg_images(tmp_path: Path) -> None:
    ...
    entry = {
        "html": (
            "<html><head><title>Nature SVG</title></head>"
            '<body><img src="https://media.nature.com/example.svg"></body></html>'
        ),
        ...
        "images": [
            {
                "image_file": image_file,
                "image_url": "https://media.nature.com/example.svg",
                "image_url_ori": "https://media.nature.com/example.svg",
...
def test_ingest_without_output_dir_yields_remote_batches(tmp_path: Path) -> None:
    ...
    assert batch["text"]["data"] == [
        "<html><head><title>Example | Nature</title></head><body>hello</body></html>",
    ]
    assert batch["text"]["tags"] == [["NATURE_ARTICLE"]]
    text_info = json.loads(batch["text"]["info"][0])
    assert text_info["url"] == "https://www.nature.com/articles/example"
